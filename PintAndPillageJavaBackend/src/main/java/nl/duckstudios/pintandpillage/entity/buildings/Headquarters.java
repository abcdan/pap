...
public class Headquarters extends Building implements IStorable, IPopulatable {
    ...
    @Getter
    @Setter
    public String name = "Headquarters";

    @Setter
    @Getter
    private int moralePerHour;

    @Setter
    private int populationCapacity;
    @Setter
    private int resourceCapacity = 750;
    @Setter
    @Getter
    private LocalDateTime lastTimeMoralUpped;
    ...
    public void updateBuilding() {
        super.setConstructionTimeSeconds(30 + ((45 + (super.getLevel() - 1) * 2L) * (long) Math.pow(super.getLevel(), 1.75)));
    ...
    protected void updateVillageState() {
        ...
        super.getVillage().setMorale(Math.min(100, super.getVillage().getMorale() + amountMoraleGained));
        this.lastTimeMoralUpped = LocalDateTime.now();
    }
}
